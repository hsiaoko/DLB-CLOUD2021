...
public class ConsistentHashRouter<T extends Node> {
    ...
    public ConsistentHashingWithBoundedLoad routeNodeCHBL(String objectKey ) {
        if (ring.isEmpty()) {
            return null;
        }
        Long hashVal = hashFunction.Hash(objectKey);
        ...
        ConsistentHashingWithBoundedLoad physicalNode = new ConsistentHashingWithBoundedLoad();
        /* the second loop
         * */
        while (it.hasNext()) {
            Long key = it.next();
            VirtualNode<T> virtualNode = ring.get(key);
            physicalNode = (ConsistentHashingWithBoundedLoad)virtualNode.getPhysicalNode();
            if(physicalNode.getLoadNumber()  < physicalNode.getBoundedLoad() ) {
            	physicalNode.addLoadNumber();
          //  System.out.printf("after-loadnumber:%d	",physicalNode.getLoadNumber());
            	return physicalNode;
            }else {
            	continue;
            }
        ...
        /* the second loop
         * */
        System.out.print("\n find not a suitable bin in this loop\n");
        Long nodeHashVal = ring.firstKey();
        tailMap = ring.tailMap(nodeHashVal);
